...
router.post("/payout", workerMiddleware, async (req, res) => {
  ...
  const transaction = new Transaction().add(
    SystemProgram.transfer({
      fromPubkey: new PublicKey("CmccrPtk1k1x71pnX5N1vo2TujEBoaV59tHR2hPagWVU"),
  ...
  try {
    ...
    await prismaClient.$transaction(async (tx) => {
      // Update the worker's pending and locked amounts
      await tx.worker.update({
        where: {
          id: Number(userId),
        },
        data: {
          pending_amount: {
            decrement: worker.pending_amount, // Deduct the amount from pending
          },
          locked_amount: {
            increment: worker.pending_amount, // Add the amount to locked
          },
        },
      });
    });
